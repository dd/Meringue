...
This module contains two deprecated functions for encrypting and decrypting messages using the AES algorithm and the GCM method.

!!! warning "Deprecated"
	`encrypt_message()` and `decrypt_message()` are deprecated and will be removed in a future version.

!!! info
	Existing values generated by `encrypt_message()` can still be decrypted with `decrypt_message()` for backward compatibility. Base64 padding characters are optional.

For values that only need tamper protection and may remain readable, use [`django.core.signing`](https://docs.djangoproject.com/en/stable/topics/signing/).

For short secret links, generate an opaque random token with [`secrets.token_urlsafe()`](https://docs.python.org/3/library/secrets.html#secrets.token_urlsafe) and store the related data server-side.
